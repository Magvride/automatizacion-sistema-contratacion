...
2. Crea la carpeta `tests/` con:
   ...
   - **`tests/test_merge_alfresco.py`**: prueba `main.fase_merge_alfresco(None, salidas_tmp)` creando `verificacion_alfresco.csv` (con `alfresco`/`cantidad_archivos`) y el consolidado `02_conciliacion_UISARD_NUEVAS_VERSIONES.csv` en `tmp_path`; verifica que el 02 queda con las columnas `alfresco` y `cantidad_archivos`, que las filas sin expediente quedan en blanco y las no encontradas en `NO`.
